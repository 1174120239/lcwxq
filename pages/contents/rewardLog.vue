			...
			<view class="cu-item" v-for="(item,index) in userList" :key="index">
				...
				<view class="content">
					<view class="text-grey">
						<text class="text-black text-bold">{{item.userJson.name}}</text>
				...
				<view class="action goUserIndex"  @tap="toUserContents(item.userJson)">
					<view class="cu-btn bg-gradual-red">主页</view>
					
				</view>
			</view>
			<view class="load-more" @tap="loadMore" v-if="userList.length>9">
				<text>{{moreText}}</text>
			</view>
