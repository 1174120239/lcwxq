	...
	<view class="user campus-subpage campus-auth-page" :class="{'campus-night': campusNight}">
		...
		<view class="auth-content">
			<view class="auth-intro">
				<CampusAuthBrand caption="1942 · 欢迎回到校园社区"></CampusAuthBrand>
				<view class="t-login"><view class="t-b">欢迎回来</view></view>
				<text class="auth-subtitle">登录后继续查看校园动态与消息</text>
			</view>
			<view class="user-form">
				<form>
					<view class="cu-form-group auth-field">
						<text class="auth-field-icon cuIcon-people"></text>
						<input name="input" placeholder="请输入账号" v-model="userName" confirm-type="next"></input>
					</view>
					<view class="cu-form-group auth-field">
						<text class="auth-field-icon cuIcon-lock"></text>
						<input name="input" placeholder="请输入密码" type="password" v-model="password" confirm-type="done" @confirm="login"></input>
					</view>
					<view class="auth-form-meta">
						<text @tap="toFoget">忘记密码？</text>
					</view>
					<view class="user-btn flex flex-direction">
						<button class="cu-btn auth-primary lg" @tap="login">立即登录</button>
						<button class="cu-btn auth-secondary lg" @tap="toRegister">注册新账号</button>
					</view>
				</form>
			</view>

		<!-- #ifdef APP-PLUS -->
			<view class="auth-other" v-if="(qqlogin + wxlogin + wblogin) > 0">
