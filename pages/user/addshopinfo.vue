<template>
	<view class="post campus-subpage campus-editor-page" :class="AppStyle">
		...
		<form>
			<view class="cu-form-group margin-top">
				...
				<input :placeholder="placeholderText" type="number" name="input" v-model="price"    @focus="ToisText(0)" @blur="ToisText(0)" :adjust-position="false" :focus="false"></input>
				<!--<view class="action">
			...
			<view class="cu-form-group">
				<view class="title">商品封面
				</view>
				<input placeholder="" name="input" v-model="imgurl" disabled="disabled"></input>
				<view class="action">
					<text class="cu-btn bg-blue radius" @tap="imgUpload">上传封面</text>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		onLoad() {
		  ...
		  computed: {
		    placeholderText() {
		      // 根据从服务器接收到的 JSON 数据生成默认文本
		      return `以${this.assetsname}为单位，VIP用户${this.vipzkname}`;
		    },
		  },
		  mounted() {
		  			 this.getset();
				  ...
				  var that = this;
				      uni.request({
				        ...
				        success(res) {
							...
							that.vipzkname = res.data.vipzkname;
