			...
			<view class="cu-item" v-for="(item,index) in userList" :key="index" @tap="toUserContents(item.userJson)">
				...
				<view class="content">
					<view class="text-black">
						...
						<text v-if="item.userJson.groupKey=='contributor'||item.userJson.groupKey=='administrator'" class="cuIcon-light text-shojo margin-left-xs"></text>
						<!--  #ifdef H5 || APP-PLUS -->
						<!-- <block v-if="item.userJson.isvip>0">
				...
				<view class="action goUserIndex">
					<view class="padding-xs bg-shojo round">主页</view>
