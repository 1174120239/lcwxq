	...
	<view class="invitation-page campus-page" :class="{'campus-night': campusNight}">
		...
		<view class="invitation-shell">
			<view class="invitation-hero">
				<view class="invitation-hero-glow"></view>
			...
			<view class="invitation-card dashboard-card" v-if="isLoggedIn && dashboard.inviteCode">
				...
				<view class="action-row">
					...
					<view class="secondary-action" @tap="shareSystem"><text class="cuIcon-share"></text><text>分享</text></view>
				</view>
			</view>
			...
			<view class="invitation-card download-card" v-if="config.androidDownloadUrl || config.iosDownloadUrl">
				<view class="card-heading"><view><text class="cuIcon-down"></text><text>下载客户端</text></view><text class="card-caption">进入校园社区更方便</text></view>
...
<script>
	...
	export default {
		...
		computed: {
			...
			shareLink() {
				return API.GetWebUrl() + '#/pages/user/invitation?invite=' + encodeURIComponent(this.dashboard.inviteCode || this.inviteCode)
			}
		},
		onLoad(query) {
		...
		methods: {
			...
			toRegister() { uni.navigateTo({ url: '/pages/user/register' + (this.inviteCode ? '?invite=' + encodeURIComponent(this.inviteCode) : '') }) },
