<template>
	<view class="grade">
		<view class="custom-navbar" :style="'padding-top:' + StatusBar + 'px;'">
			<view class="navbar-content" :style="'height:' + CustomBar + 'px;'">
				<view class="navbar-left" @click="goBack">
					<image src="../../static/image/travel/ranking/headLeft.png"></image>
				</view>
				<view class="navbar-center">
					<text>我的等级</text>
				</view>
				<view class="navbar-right" @tap="showModal" data-target="Modal">
					<text>等级说明</text>
				</view>
			</view>
		</view>
		<view class="top" >
			<image class="line" src="../../static/image/travel/ranking/arc.png"></image>
			<view class="num">
				<view>
					<image src="../../static/image/travel/ranking/grade.png"></image>
		...
		<view class="cont">
			...
			<view class="button-group">
			  <view class="button" :class="{'is-signed': isClock==1}" @tap="qiandao()">
			    <text>{{isClock==1?'今日已签到':'立即签到'}}</text>
			  </view>
			</view>
			</view>
			...
			that.$Net.request({
				...
				dataType: 'json',
					...
					if(res.data.code==0){
						...
						if(res.data.data && res.data.data.isClock != null){
							that.isClock = Number(res.data.data.isClock);
						}
		...
		qiandao(){
		    ...
		    token = localStorage.getItem('token');
					uni.request({
						...
						dataType:"json",
						success(res) {
						  var msg = res.data && res.data.msg ? res.data.msg : '';
						  if(res.data && (res.data.code==1 || res.data.code==200 || msg.indexOf('成功')!=-1 || msg.indexOf('已')!=-1)){
							that.isClock = 1;
							uni.$emit('campus-signin-updated');
						  }
						  uni.showToast({
						    title: msg || '签到完成',
						    icon: 'none'
			...
			that.$Net.request({
				...
				success: function(res) {
					if(res.data.code==1){
						...
						if(profileData.isClock != null){
							that.isClock = Number(profileData.isClock);
						}
					}
				}
			})
	...
	.grade{
		...
		.cont{
			...
			.experience{
				...
				.button.is-signed{
					background: linear-gradient(90deg, #aab7b3 0%, #7f918c 100%);
				}
			}
			.experience2{
				width: 100%;
