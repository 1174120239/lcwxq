<template>
	<view class="user campus-subpage campus-chat-page" :class="AppStyle">
		...
		<view class="cu-chat" >
			...
			<view class="cu-info" v-if="isFollow==0">
				...
				<text class="text-blue" @tap="follow(1)">关注</text>
			</view>
			<block v-for="(item,index) in msgList">
				...
				<view class="cu-item" :class="item.uid==uid?'self':''" v-if="item.type!=4">
					...
					<view class="main" :style="item.type==1?'background-color: #00000000;':''">
						
						<block v-if="item.type==0">
							<view :class="item.uid==uid?'':''" @longtap="ToCopy(item.text)">
...
<script>
	...
	export default {
		...
		onLoad(res) {
			...
			// #endif
			const routeType = res.type == 1 || res.type == '1' ? 1 : 0;
			if ((routeType == 1 && !featureFlags.groupChat) || (routeType == 0 && !featureFlags.privateChat)) {
				uni.showToast({
					title: '该功能暂未开放',
					icon: 'none',
					duration: 1200,
		...
		methods: {
			...
			formatDate(datetime) {
			    ...
			    var year = datetime.getFullYear(),
			        month = ("" + (datetime.getMonth() + 1)).slice(-2),
			        date = ("" + datetime.getDate()).slice(-2),
			       ...
			       var result = "";
			        ...
			        if (datetime.toDateString() === now.toDateString()) {
			            // 判断时间段
			            if (hour < 6) {
			                result = "凌晨 " + hour + ":" + minute;
			            } else if (hour < 12) {
			                result = "上午 " + hour + ":" + minute;
			            } else if (hour < 18) {
			                result = "下午 " + (hour - 12) + ":" + minute;
			            } else {
			                result = "晚上 " + (hour - 12) + ":" + minute;
			            }
			        }
			        // 判断是否是昨天
			        ...
			        // 判断是否是一年以内
			            ...
			            if (now.getTime() - datetime.getTime() < 7 * 24 * 3600 * 1000) {
			                ...
			                result = weekday[datetime.getDay()] + " " + hour + ":" + minute;
			            } else {
