	...
	<view class="userpost" :class="AppStyle">
		<view class="header" :style="[{height:CustomBar + 'px'}]">
			<view class="cu-bar bg-white" :style="{'height': CustomBar + 'px','padding-top':StatusBar + 'px'}">
				...
				<!--  #ifdef H5 || APP-PLUS -->
				<view class="action" @tap="toAdd(type)">
		...
		<view class="data-box">
			...
			<view class="all-btn">
				<view class="user-btn flex flex-direction">
					<button class="cu-btn bg-blue margin-tb-sm lg" @tap="toAdd(type)">{{type=='tag'?'新增话题':'新增分类'}}</button>
...
<script>
	...
	export default {
		...
		onLoad(res) {
			...
			// #endif
			if (res && res.type) {
				that.type = res.type === 'tag' ? 'tag' : 'category';
			}
		},
		onShow(){
			var that = this;
		...
		methods: {
			...
			toAdd(metaType){
				var that = this;
				var targetType = metaType || that.type || 'category';
				
				uni.navigateTo({
					url: '/pages/manage/metasedit?type=add&metaType=' + targetType
				});
			},
			searchTag(){
				var that = this;
