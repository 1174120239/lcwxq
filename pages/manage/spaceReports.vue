	...
	<view class="report-manage" :class="AppStyle">
		<view class="header" :style="[{height:CustomBar + 'px'}]">
			<view class="cu-bar bg-white" :style="{'height':CustomBar + 'px','padding-top':StatusBar + 'px'}">
				...
				<view class="content text-bold" :style="[{top:StatusBar + 'px'}]">动态举报审核</view>
		...
		<view class="report-summary">
			<text class="report-summary-title">待处理举报</text>
		...
		<view class="report-empty" v-if="!loading && list.length===0"><text class="cuIcon-check"></text><text>没有待处理举报</text></view>
		...
		methods: {
			...
			loadList(append,complete){
				...
				this.$Net.request({
					...
					data: { token: this.token, status: 0, page: targetPage, limit: this.pageSize },
					method: 'get',
					dataType: 'json',
					success: (res) => {
...
<style scoped>
	...
	.campus-night .report-summary,.campus-night .report-item { border-color: #303a37; background: #1d2523; }
