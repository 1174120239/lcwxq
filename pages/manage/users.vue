			...
			<view class="cu-item" v-for="(item,index) in userList" :key="index" >
				...
				<view class="action user-list-btn">
					<block v-if="type==''">
						...
						<block v-else>
							<view class="cu-btn text-red radius" @tap="deleteUser(item.uid)"  v-if="item.uid>1&&group=='administrator'">
								<text class="cuIcon-deletefill"></text>
							</view>
							<view class="cu-btn text-blue radius" @tap="toEdit(item.uid)"  v-if="item.uid>1&&group=='administrator'">
								<text class="cuIcon-post"></text>
							</view>
							
							<view class="cu-btn text-black radius" @tap="toBan(item.uid)" v-if="item.uid>1&&group=='administrator'">
					...
					<block v-if="type=='get'">
						<view class="cu-btn text-blue radius" v-if="item.uid>1" @tap="getUser(item)">
							选择
						</view>
					</block>
				</view>
