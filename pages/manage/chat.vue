	...
	<view :class="AppStyle">
		<view class="header" :style="[{height:CustomBar + 'px'}]">
			<view class="cu-bar bg-white" :style="{'height': CustomBar + 'px','padding-top':StatusBar + 'px'}">
				...
				<view class="action" v-if="groupChatEnabled">
		...
		<view class="data-box data-inbox">
			<view class="search-type grid col-2">
				<view class="search-type-box" v-if="groupChatEnabled" @tap="toType(1)" :class="type==1?'active':''">
					<text>全站群聊</text>
				</view>
				<view class="search-type-box" v-if="group=='administrator'&&privateChatEnabled"  @tap="toType(0)" :class="type==0?'active':''">
					<text>全站私聊</text>
				</view>
			</view>
			<block v-if="type==1&&groupChatEnabled">
			...
			<block v-if="type==0&&group=='administrator'&&privateChatEnabled">
				<view class="no-data" v-if="chatList.length==0">
					暂时没有数据
				</view>
				<view class="cu-list menu-avatar"  v-if="chatList.length>0">
...
<script>
	...
	export default {
		...
		onShow(){
			var that = this;
			if (!that.privateChatEnabled && !that.groupChatEnabled) {
				that.rejectDisabledChatManage();
				return false;
			}
			that.page=1;
			// #ifdef APP-PLUS
		...
		methods:{
			...
			toType(i){
				var that = this;
				if ((i == 1 && !that.groupChatEnabled) || (i == 0 && !that.privateChatEnabled)) {
					return that.rejectDisabledChatManage();
				}
				that.chatList = [];
				that.type=i;
				that.page=1;
			...
			addGroup(){
				if (!this.groupChatEnabled) {
					return this.rejectDisabledChatManage();
				}
				var that = this;
				uni.navigateTo({
