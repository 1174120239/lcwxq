<template>
	<view class="campus-page campus-square" :class="{'campus-night': campusNight}">
		...
		<block v-if="squareid==0&&contentMode==='space'&&follow==1&&selectedTopics.length===0">
			...
			<view class="space-presentation" v-if="showSpacePresentation && (!feedTypeFilter || feedTypeFilter==='space') && (presentationBanners.length>0 || presentationPins.length>0)">
		...
		methods: {
			...
			loadFeed(append, complete) {
				...
				if (!append) {
					...
					this.feedError = '';
					if (!this.feedTypeFilter || this.feedTypeFilter === 'space') {
						this.loadSpacePresentation();
					} else {
						this.presentationRequestId++;
						this.presentationBanners = [];
						this.presentationPins = [];
					}
				}
				...
				this.$Net.request({
					...
					success: (res) => {
						...
						const list = this.excludePresentedFeedItems(sourceList);
						this.feedList = append ? this.feedList.concat(list) : list;
						this.feedPage = targetPage;
						this.feedTotal = Number(res.data.total || 0);
						this.feedError = '';
						this.feedMoreText = sourceList.length === 12 && this.feedList.length < this.feedTotal
							? '上滑或点击加载更多' : '已经到底了';
					},
					fail: () => {
						if (requestId !== this.feedRequestId) return;
			...
			excludePresentedFeedItems(rows){
				if(!this.showSpacePresentation || (this.feedTypeFilter && this.feedTypeFilter !== 'space')) return rows;
				var ids = this.presentationIds();
				if(!ids.length) return rows;
				return (rows || []).filter(function(item){
					return item.feedType !== 'space' || ids.indexOf(String(item.id)) === -1;
				});
			},
				...
				that.$Net.request({
					...
					success:function(res){
						...
						if(res.data.code==1){
							...
							that.feedList = that.excludePresentedFeedItems(that.feedList);
						}
					},
					fail:function(){
