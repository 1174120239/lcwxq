...
- 迁移 010 新增个人资料性别/生日独立公开设置、匿名投票、可配置 DeepSeek 动态审核和动态分析事件表。消息中心支持单条已读、全读和未读红点。
- 投票为单选或限项多选，结果只显示计数和比例，不返回参与者身份。AI 审核默认关闭；风险、模型异常及带附件动态进入举报审核页人工处理。
- PHP 后台新增 AI 动态审核配置、论坛动态数据总览和用户论坛数据钻取。总览包含注册后 7 天发帖率、动态回复率、零回复率、注册/发布/回复趋势、用户漏斗、活跃时段、审核及举报指标；用户页可按 UID、用户名或昵称检索，查看首次发动态耗时、发布与回复、收到直接回复、获赞、浏览和最近行为。分析只使用可追溯数据，事件不足时不估算留存率，也不展示账号凭据或联系方式。
...
| 010_dynamic_core_extensions.sql | 性别/生日公开设置、动态投票、AI 审核记录和动态分析事件 |
...
| log | 收藏、互动日志、订单读取和清理 |
| space | 动态读写、审核、锁定、删除、点赞、关注、浏览量、话题、用户举报和 staff 举报审核 |
| qa | 用户提交待审核问题、回答、点赞回答、评论和回复；staff 审核管理问题；问答站内通知与 UniPush |
...
- 管理控制台为 administrator/editor 提供“举报审核”，可驳回举报或删除原动态；处理结果记录审核人、状态、说明和时间，删除动态时同步关闭该动态的其他待处理举报。
...
站内通知仍保留；动态评论和回复会写入 `starfree_inbox` 的 `spaceComment` 类型并携带原动态 id，消息中心可直接打开动态详情；问答新回答写入 `qaAnswer`，回答评论和评论回复写入 `qaComment`，消息中心与 UniPush 点击可打开对应问题。动态点赞只记录点赞日志和计数，不产生站内信、推送或邮件通知。UniApp Push 使用用户绑定的 `clientId`，后端 UniPush 发送器由 `UNIPUSH_ENABLED` 和运行时凭据控制，支持 `unipush.protocol=v1`（UniPush 1.0 / 个推 REST v1，`auth_sign` + `push_single`）与 `v2`（个推 REST v2）两种协议，生产按 App 打包版本选择 v1。推送失败不影响站内消息落库。私聊和群聊前端入口已收敛或隐藏。旧聊天接口未重建，不能把消息中心超时直接归因于新后端。
...
- App 首页显示时通过后台 `Api/api.php?update=1` 检查最新版本，版本码按整数比较；非 App 端不调用 `plus.runtime`。
- 后台 `force=1`（接口字段 `qzgx=1`）显示不可关闭的“强制更新”弹窗，用户返回 App 后仍保留；普通更新显示“稍后再说”，本次 App 运行期间不重复提示同一版本。
- 设置页“检查更新”复用同一版本解析规则，并在确认框标题中明确显示“强制更新”或“普通更新”；下载页打开失败时复制后台配置的下载链接。
...
| admin/starfree-admin/tests/dynamicAnalyticsCommonTest.php | 后台论坛统计百分比、数字格式和 UTF-8 摘要降级 |
