...
| qa | staff 管理问题；用户回答、点赞回答、评论和回复；问答站内通知与 UniPush |
...
- QQ 动态助手使用个人 QQ 号登录 NapCat，再通过 OneBot v11 反向 WebSocket 接 AstrBot；不是 QQ 官方机器人号。后端 `SFreeBot/*` 只开放动态工具，不开放帖子/文章能力。QQ 空间每日同步将每条公开已审核动态生成一张 `P1-P9` 编号图片，通过 NapCat `send_qzone_msg` 发布一条最多九图的说说；发布时间和图片模板由 PHP 后台配置，成功后才推进独立游标。
- 首页帖子流下方显示独立的“校园问答”问题卡片；点击进入问题详情，回答、回答评论和评论回复均在当前页完成。
- 普通用户没有提问入口。管理员和编辑从管理控制台的“问答管理”新增、编辑、排序、推荐、发布或停用问题。
...
消息、聊天、动态及用户列表统一使用 `campus-avatar` 展示头像。头像字段为空、用户已注销或图片加载失败时显示姓名首字占位，不得留下空白头像区域。
...
- 管理端帖子、分类、话题和轮播设置。
- 首页问答卡片、问题详情、回答排序、点赞、回答评论/回复，以及管理端问题新增、编辑和停用。
- 桌面宽屏与移动端。
- 日间和夜间主题。
