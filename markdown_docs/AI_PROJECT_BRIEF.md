...
| feed | 只读社区动态流：聚合公开动态、普通帖子、校园问答和未过期互助任务；刊物文章不混入，问题卡展示最新回答摘要 |
| proxy | 未迁移接口和受控旧端委托 |
...
- 动态页“全部”视图使用 `SFreeFeed/feedList` 统一推送原动态、普通帖子、校园问答和校园互助任务；普通帖子排除“见字”刊物文章，问题按问题更新时间或最新回答时间排序并显示一条最新回答摘要，互助任务显示求助/提供帮助和当前状态。卡片分别进入原有详情页；话题、关注、视频、图集和精华筛选仍使用动态接口，避免改变既有筛选语义。首页继续保留独立的推荐问答模块，提问区仍可查看完整问题列表。
