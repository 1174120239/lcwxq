...
- QQ 动态助手使用个人 QQ 号登录 NapCat，再通过 OneBot v11 反向 WebSocket 接 AstrBot；不是 QQ 官方机器人号。后端 `SFreeBot/*` 只开放动态工具，不开放帖子/文章能力。QQ 空间每日同步把一批公开已审核动态合成一张图片，通过 NapCat `send_qzone_msg` 发布；发布时间和图片模板由 PHP 后台配置，成功后才推进独立游标。
