...
| feed | 只读社区动态流：聚合公开动态、校园问答和未过期互助任务；刊物文章不混入，问题卡展示最新回答摘要 |
| proxy | 未迁移接口和受控旧端委托 |

### 8.2 混合处理

- contentsAdd/contentsUpdate 只在普通 post/video 场景由新端完整写入；付费、草稿、商品关联和未知类型可委托旧端。contentsAdd 的 `journal=1` 会把文章标记为“见字”投稿，普通用户固定进入 waiting，公开列表用 `searchParams.journal=1` 筛选已发布投稿。
...
- 内容频道新增“见字”入口：主页保留动态主流，右上和内容区提供见字阅读入口；见字使用独立刊物和文章表，不复用普通帖子。用户从刊物详情进入投稿，非 staff 投稿固定进入待审核状态，审核通过后才出现在见字列表。管理员可审核、隐藏、恢复和软删除文章；软删除状态为 `deleted`，保留正文并写入见字操作日志，公开列表只返回 `published`。
...
- 动态页“全部”视图使用 `SFreeFeed/feedList` 统一推送原动态、校园问答和校园互助任务；问题按问题更新时间或最新回答时间排序并显示一条最新回答摘要，互助任务显示求助/提供帮助和当前状态。卡片分别进入原有详情页；话题、关注、视频、图集和精华筛选仍使用动态接口，避免改变既有筛选语义。统一接口尚未切流时，前端回退读取现有三类公开列表，避免动态页显示加载失败。首页继续保留独立的推荐问答模块，提问区仍可查看完整问题列表。
...
- 四个主 Tab（首页、动态、消息、我的）只通过底部导航切换；页面级左右滑动切换已移除，避免正常滚动和操作时误触。
...
测试覆盖控制器兼容、权限、审核、幂等、Redis 登录态、MyISAM 补偿、动态可见性、动态精华/置顶权限与有效期查询、内容委托、匿名动态权限与隐私边界、普通用户问题待审核提交、动态举报去重与 staff 处理、社区动态多来源排序/最新回答/类型筛选与分页边界、经济操作、轻量邀请奖励、校园互助 Lv 门槛/所有权/状态流转/公开评论/QQ 接收者投影和评论删除撤销授权、评论/回复/联系方式分享的站内信/推送/邮件通知与重复分享幂等，以及邮箱验证码协议、限流、SMTP 异常分类、认证退避和失败清理。
...
- 主页“动态 / 发现”层级、见字入口和见字页专栏筛选；普通用户从“投稿见字”进入文章编辑器后，提交请求带 `journal=1` 并显示投稿状态，staff 审核通过后文章才公开。
