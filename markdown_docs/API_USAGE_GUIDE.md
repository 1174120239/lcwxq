...
| 路径 | 方法/鉴权 | 参数 | 路由 | 调用与注意点 |
|---|---|---|---|---|
| `SFreeUsers/inbox` | GET/POST / token | `type,page,limit` | 旧端 | 读取不会自动设为已读。 |
| `SFreeUsers/unreadNum` | GET/POST / token | `token` | 旧端 | 不包括旧聊天未读数。 |
| `SFreeUsers/setRead` | GET/POST / token | `type` | 旧端 | `all/comment/finance/system/fan`；chat 为历史兼容，返回 0；可重复调用。 |
