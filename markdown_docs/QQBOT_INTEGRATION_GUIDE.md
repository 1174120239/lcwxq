...
所有插件调用都带 `botSecret` 和 `platform=qq`，返回旧协议 `{code,msg,data}`。普通请求使用 `application/x-www-form-urlencoded`；带图动态的 `addSpace` 使用 `multipart/form-data`，图片字段名为可重复的 `images`。
...
| `POST /SFreeBot/addSpace` | 发动态或评论动态 | 发动态使用 `qqUserId,requestId,text,pic,topicIds,onlyMe`；带图时重复提交 `images` 文件字段；评论额外使用 `type=3,toid=<动态ID>` |
...
发动态和修改资料是敏感写操作，插件会先生成预览。图文动态支持同一条消息发送，也支持文字、图片分多条任意顺序发送；插件会在确认前持续合并草稿并刷新预览，用户回复 `确认`、`确认发布`、`确认修改`、`发吧`、`继续` 或 `可以` 后才执行。取消词支持 `取消`、`算了`、`不发了`、`不改了`，会清空整份动态草稿。
...
用户询问论坛下载、安装、论坛链接或邀请链接时，插件直接返回固定邀请下载页
`https://prev.lcxqy.cn/#/pages/user/invitation?invite=LY4898VS95`，不经过模型生成，避免链接被改写或编造。
...
- `立刻发布未同步动态` 按钮生成一次性任务，只绕过当天时间限制，仍从 `qzone_cursor_space_id` 之后读取，避免重新发送已发布动态；不足批量上限时按实际数量发布，成功后才完成任务，失败保留并重试。
...
- 发布成功后记录 `qzone_cursor_space_id`、`qzone_last_run_date`、`qzone_last_tid`、`qzone_last_success_at`。普通同步和“立刻发布”使用同一个空间动态游标，不会重发游标之前的动态。
