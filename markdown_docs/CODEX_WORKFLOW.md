...
数据库迁移不属于普通一键发布。只有先按 `DEPLOYMENT_GUIDE.md` 审查迁移和备份数据库后，才可在独立维护任务中执行。当前 `-RunMigrations` 仅对白名单中的迁移 014、015、016 和 `replacement-backend` 生效；入口会固定校验 SQL SHA-256、备份受影响表并验证结构。任何其他组件或迁移请求都会在生产变更前中止。
